# 夜のネオン街をイメージしたポートフォリオ

夜のネオン街をイメージした、黒を基調にしたポートフォリオサイト。制作物はネオン看板を模したカードで並べています。
派手な演出を持ちながら Lighthouse 95+ を維持することそのものが、このサイトの主張です。

> **サイトの名前は `Mawo's Portfolio` です**（`src/lib/site.ts`）。
> タブのタイトル・`og:site_name`・構造化データの `WebSite.name`・ヘッダーの表示はすべてこれを使います。
> 「夜のネオン街」は**デザインのテーマ**であって、外に出す名前ではありません。
> 検索やSNSで出会う人には「誰のポートフォリオか」が分かる名前のほうが役に立つためです（仕様書 §11）。
> ヒーローとOG画像にも同じ名前を出します（2行に割る都合で文字列は直接書いています）。
...
`public/fonts/` のサブセット済み woff2 だけがリポジトリに含まれます。
現在の合計サイズは `src/data/metrics.json` で自動計測しています。
...
| `npm run budget:check` | 4ページ×3回の中央値で §8 の予算を検証 |
...
**フェーズ3（静的な成果物看板）の時点でサイトとして完成しています。**
...
初期JSは **30KB (gzip) 以下**を設計上限とし、実測値は `src/data/metrics.json` に保存します。
直接読むモジュールには Astro の ClientRouter とアイランド起動用ブートストラップだけを載せます。
...
- **全ページ共通で preload するのは Archivo だけ**。ページ固有の LCP 要素に別の書体が
  必要な場合だけ `Base.astro` の `preloadFonts` で追加します。現在は `/about-this-site/` の
  導入文用に Zen Kaku Gothic New Regular の core を追加しています。IBM Plex Mono は
  先読みしません。
...
| URL | 役割 |
| :-- | :-- |
| `/` | 入口。ヒーロー → プロフィール → 経歴 → 成果物と「このサイトについて」への看板 |
...
`src/content/works/images/<slug>.png` に**1枚だけ**置きます（16:9 前後）。
一覧のサムネイルと詳細ページのカバーで同じファイルを使い回し、表示側の
`object-fit: cover` で切り抜くため、素材の比率が多少ずれても崩れません。

> **カバーは 800px / quality 50 を超えないでください。** 実測で決めた上限です。
> このカバーは作品詳細ページの LCP 要素そのもので、大きくすると即座に予算を割ります。
>
> | 設定 | 転送量 | LCP |
> | :-- | --: | --: |
> | カバー無し | — | 1802ms |
> | **800px q50** | **19.9KB** | **1952ms** |
> | 960px q55 | 26.9KB | 2102ms（予算割れ） |
> | 1200px q62 | 41.8KB | 2101ms（予算割れ） |
>
> 26.9KB と 41.8KB が同値なのは、どちらも同じ RTT の枠に落ちるためです。
> **20KB あたりに境目**があり、越えると往復が1回増えます。
> 同じ理由で q42（16.3KB）も 1952ms のままなので、枠の中で最も画質の高い q50 を採っています。
> 実際に表示されるのは `.work` の 72ch（約576px）までなので、それ以上配っても見えません。
...
| CLS | 0.05 以下 |
| TBT | 200ms 以下 |
| 初期JS (gzip) | 30KB 以下（設計上限） |
| フォント合計 | 200KB 以下 |

最新のトップページ実測値は `src/data/metrics.json` に保存し、`/about-this-site/` に表示します。
...
> `/works/` では和文 Regular の preload を**採用しませんでした**。手元の CLS は 0.019 のまま変わらず、
...
> 増やす変更をしたら、`npm run budget:check` で4ページとも予算内か確認してください。
